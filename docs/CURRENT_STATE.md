...
当前并行推进：
- Phase 0.5-A Harness Integration Spike：PR #2 / `spike/harness-integration`；
- Human Acquisition baseline amendment：`docs/human-acquisition-baseline`，用于在 MVP 开始前冻结 HumanSubmission 入口；
...
PR #2 必须真实回答：
- exact-pin Harness 是否可稳定启动；
- out-of-tree Profile/Plugin/Tool 是否可维护；
- Tool → FastAPI 边界是否工作；
- Opportunity Card / Research Job / durable Conversation Node 是否可用；
- Harness 是否足以承载复杂 Workbench，或应冻结 `HYBRID_WEB_HARNESS`。
