...
当 exact-head CI 验证 D1 runner 后，用户只需要先运行 D1 no-key baseline 并提供生成 artifact 或关键摘要。D1 通过后，再决定是否申请/配置 Exa、Firecrawl、Tavily 对应 API key 执行 D2；不要求一次性准备全部 secret。
