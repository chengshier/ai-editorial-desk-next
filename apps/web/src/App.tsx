...
const primaryItems = [
  ['/today', '今日'], ['/opportunities', '机会'], ['/programming', '编排'],
  ['/creation', '创作'], ['/publication', '发布'], ['/performance', '表现'], ['/knowledge', '知识'],
] as const

const todayViews = [
  ['全部机会', Sparkles], ['正在升温', Flame], ['值得一看', Compass],
  ['潜力发现', CircleHelp], ['长期储备', Leaf], ['正在研究', FolderKanban],
] as const
  ...
  return <header className="top-nav">
    <div className="top-nav__left">
      ...
      <nav className="primary-nav" aria-label="全局产品导航">
        {primaryItems.slice(0, 2).map(([to, label]) => <NavLink key={to} to={to} className={({isActive}) => `primary-nav__item${isActive ? ' is-active' : ''}`}>{label}</NavLink>)}
        <span className={`primary-nav__item primary-nav__item--disabled${location.pathname.startsWith('/research/') ? ' is-active' : ''}`} title="研究页需要从具体 Research Case 进入">研究</span>
        {primaryItems.slice(2).map(([to, label]) => <NavLink key={to} to={to} className={({isActive}) => `primary-nav__item${isActive ? ' is-active' : ''}`}>{label}</NavLink>)}
      </nav>
    </div>
    ...
    <div className="sidebar__scroll">
      <section className="sidebar-section">
        ...
        <nav className="sidebar-list">
          {todayViews.map(([label, Icon], index) => <NavLink key={label} to="/today" title={label} className={`sidebar-link${isToday && index === 0 ? ' is-active' : ''}`}><Icon size={15}/><span>{label}</span></NavLink>)}
        </nav>
      </section>
      <section className="sidebar-section">
...
function ResearchPage() {
  ...
  return <WorkspaceLayout eyebrow="P03 · Research Outer Route" title="研究" badge="Hybrid" description="Web Shell 拥有业务 URL；Harness 拥有 Agent / Tool / Replay / Research Workspace 内部交互。"><HarnessSurfaceHost intent="research" researchCaseId={researchCaseId} returnPath={`/research/${encodeURIComponent(researchCaseId)}`} title="Research Workspace 宿主边界"/></WorkspaceLayout>
}
