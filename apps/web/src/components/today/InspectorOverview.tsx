import { CircleHelp, Clock3, Compass, Layers, Users } from 'lucide-react'
...
export function InspectorOverview({ item }: { item: OpportunitySummary }) {
  const unknownCount = item.evidence_state.open_unknown_count
  ...
  return <>
    <section className="inspector-decision">
      ...
      <div className="inspector-perspective">
        ...
        <div><span><Users size={14}/>受众承诺</span><p>{item.audience_promise}</p></div>
    ...
    <section className="inspector-section">
      ...
      <dl className="inspector-kpis">
        ...
        <div><dt>开放未知项</dt><dd className={unknownCount > 0 ? 'is-attention' : 'is-positive'}>
          {unknownCount}<small> 项待澄清</small>
        </dd></div>
      </dl>
    </section>
    ...
    <section className="inspector-section">
      ...
      </ul> : <p className="inspector-section__empty">当前尚未提供编辑价值亮点。</p>}
    </section>
    ...
    <section className="inspector-section">
      ...
      <div className="inspector-questions">
        <div className="inspector-question">
          <CircleHelp size={14}/>
          <span>{unknownCount > 0
            ? `仍有 ${unknownCount} 个开放未知项；具体问题尚未通过 Web Shell 的 canonical contract 暴露，进入研究工作区后继续补齐和核验。`
            : '当前 Opportunity 没有开放未知项；后续研究仍以正式 Evidence contract 为准。'}</span>
        </div>
      </div>
    </section>
  </>
