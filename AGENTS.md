...
- 至少支持 `ambient / potential / momentum / research` 四类 discovery lane。
...
Architecture + Functional Baseline v1 尚未进入完整业务实现阶段。

允许：项目骨架、文档、接口占位、测试骨架、最小健康检查、Spike 所需 mock/adapter/benchmark harness。

禁止：
- 提前实现完整 Subject/Discovery/Opportunity 工作流；
- 为 Spike 建立不可逆正式业务表；
- 在 Harness Spike 前假定复杂 Dashboard 一定能由 Harness 完整承担；
