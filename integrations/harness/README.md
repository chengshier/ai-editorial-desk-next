...
该 package 会在验证时复制进 **独立的 pinned Harness checkout**；本仓库不提交 Harness 源码。
...
→ host/client bundle
→ artifact assertion
→ FastAPI + Harness Web overlay boot smoke
```

自动化通过只证明 **编译/打包/Host 启动边界兼容**。以下仍需要真实 Web/Agent 人工验收后才能写 PASS：
