...
def _safe_replace(source: Path, target: Path) -> None:
    if target.exists():
        ...
        if not package_json.exists():
            raise SystemExit(f"refusing to replace non-editorial-shell directory: {target}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepare the formal AI Editorial Desk Harness product shell in an exact-pinned Harness checkout."
    )
